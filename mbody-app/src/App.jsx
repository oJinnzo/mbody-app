...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .hamburger{background:transparent;border:none;color:${C.text};font-size:22px;cursor:pointer;padding:4px 10px;border-radius:8px;transition:background .15s;display:flex;align-items:center;justify-content:center;}
  .hamburger:hover{background:${C.surface};}
  .menu-overlay{position:fixed;inset:0;background:#000a;z-index:300;animation:fadeIn .15s ease;}
  @keyframes fadeIn{from{opacity:0;}to{opacity:1;}}
  .menu-panel{position:fixed;top:0;left:0;bottom:0;width:280px;max-width:80vw;background:${C.card};border-right:1px solid ${C.border};z-index:301;padding:20px;transform:translateX(0);animation:slideIn .2s ease;display:flex;flex-direction:column;}
  @keyframes slideIn{from{transform:translateX(-100%);}to{transform:translateX(0);}}
  .menu-title{font-family:'Space Grotesk',sans-serif;font-size:16px;font-weight:700;color:${C.white};margin-bottom:4px;}
  .menu-sub{font-size:11px;color:${C.muted};letter-spacing:1.5px;text-transform:uppercase;margin-bottom:24px;}
  .menu-item{padding:14px 12px;border-radius:10px;color:${C.text};display:flex;align-items:center;gap:14px;cursor:pointer;transition:background .15s;font-size:14px;font-weight:500;margin-bottom:4px;}
  .menu-item:hover{background:${C.surface};}
  .menu-item.active{background:${C.accent}15;color:${C.accent};}
  .menu-item-icon{font-size:18px;width:24px;text-align:center;}
  .menu-divider{height:1px;background:${C.border};margin:16px 0;}
  .filter-row{display:flex;gap:8px;align-items:center;margin-bottom:14px;}
  .filter-row select{flex:1;background:${C.surface};border:1px solid ${C.border};color:${C.text};padding:9px 12px;border-radius:8px;font-size:13px;font-family:'Inter',sans-serif;outline:none;}
  .filter-row select:focus{border-color:${C.accent};}
  .filter-clear{padding:9px 14px;background:transparent;border:1.5px solid ${C.border};border-radius:8px;color:${C.muted};font-size:12px;font-weight:600;cursor:pointer;font-family:'Space Grotesk',sans-serif;}
  .filter-clear:hover{border-color:${C.accent};color:${C.accent};}
  .filter-active-badge{background:${C.accent}18;border:1px solid ${C.accent}40;border-radius:6px;padding:6px 12px;font-size:12px;color:${C.accent};margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;}
...
export default function App(){
  ...
  const[deliveries,setDeliveries]=useState(()=>loadLS('mbody_deliveries')||[]);
  ...
  const navTo=(t)=>{
    ...
    setShowMenu(false);
    setShowHub(false);
    // Refresh deliveries from localStorage in case DeliveryTab updated them
    setDeliveries(loadLS('mbody_deliveries')||[]);
  };

  // Build unique property list for filter dropdowns
  const allPropertyNames=()=>{
    const set=new Set();
    inspHistory.forEach(r=>{if(r.info?.property?.trim())set.add(r.info.property.trim());});
    srHistory.forEach(r=>{if(r.property?.trim())set.add(r.property.trim());});
    deliveries.forEach(r=>{if(r.property?.trim())set.add(r.property.trim());});
    sopProperties.forEach(p=>{if(p.name?.trim())set.add(p.name.trim());});
  ...
  return(<>
    <style>{css}</style>
    <div className="app">
      <div className="header"><div className="header-inner"><div style={{display:'flex',alignItems:'center',gap:8}}><button className="hamburger" onClick={()=>setShowMenu(true)} aria-label="Menu">☰</button><Logo onClick={()=>{resetInsp();setSR(initSR());setStep('home');setShowHub(false);}}/></div><div className="step-badge">{showHub?'PROPERTIES':stepLabel()}</div></div></div>
      ...
      {showMenu&&(<>
        ...
        <div className="menu-panel" onClick={e=>e.stopPropagation()}>
          <div className="menu-title">MBody AI</div>
          <div className="menu-sub">Service Inspection</div>
          <div className={`menu-item ${showHub?'active':''}`} onClick={()=>{setShowHub(true);setShowMenu(false);}}>
            <span className="menu-item-icon">🏢</span>
            <div>
              <div>Property Hub</div>
              <div style={{fontSize:10,color:C.muted,marginTop:2}}>All records, by property</div>
            </div>
          </div>
          <div className="menu-divider"/>
          <div className={`menu-item ${tab==='inspections'&&!showHub?'active':''}`} onClick={()=>navTo('inspections')}><span className="menu-item-icon">🔍</span><div>Inspections</div></div>
          <div className={`menu-item ${tab==='service'&&!showHub?'active':''}`} onClick={()=>navTo('service')}><span className="menu-item-icon">🔧</span><div>Service Reports</div></div>
          <div className={`menu-item ${tab==='delivery'&&!showHub?'active':''}`} onClick={()=>navTo('delivery')}><span className="menu-item-icon">🚚</span><div>Deliveries</div></div>
          <div className={`menu-item ${tab==='inventory'&&!showHub?'active':''}`} onClick={()=>navTo('inventory')}><span className="menu-item-icon">📦</span><div>Parts Inventory</div></div>
          <div className={`menu-item ${tab==='sop'&&!showHub?'active':''}`} onClick={()=>navTo('sop')}><span className="menu-item-icon">📋</span><div>SOP Tracker</div></div>
          <div style={{flex:1}}/>
          <div style={{fontSize:10,color:C.muted,textAlign:'center',padding:'12px 0',letterSpacing:1}}>mbody.ai</div>
        </div>
      </>)}
      ...
      {showHub&&(
        <PropertyHub
          inspections={inspHistory}
          services={srHistory}
          deliveries={deliveries}
          sopProperties={sopProperties}
          onClose={()=>setShowHub(false)}
          onOpenRecord={(type,rec)=>{
            setShowHub(false);
            if(type==='inspection'){setViewingInsp(rec);setTab('inspections');setStep('receipt');}
            else if(type==='service'){setViewingSR(rec);setTab('service');setStep('sr_receipt');}
            else if(type==='delivery'){setTab('delivery');showToast('Open Delivery tab to view');}
          }}
          onOpenSOPPhase={(propId)=>{
            setShowHub(false);
            setTab('sop');
            showToast('Tap the property in the SOP list');
          }}
        />
      )}
      ...
      {!showHub&&<>
      {tab==='inspections'&&<>
        {step==='home'&&<div className="content"><button className="btn" style={{marginTop:0}} onClick={()=>{resetInsp();setStep('intake');}}>+ New Inspection</button>{allPropertyNames().length>0&&(<><div className="sec">Filter</div><div className="filter-row"><select value={inspFilter} onChange={e=>setInspFilter(e.target.value)}><option value="">All Properties</option>{allPropertyNames().map(n=><option key={n} value={n}>{n}</option>)}</select>{inspFilter&&<button className="filter-clear" onClick={()=>setInspFilter('')}>Clear</button>}</div></>)}<div className="sec">Saved ({inspHistory.filter(r=>!inspFilter||r.info?.property===inspFilter).length}{inspFilter?` of ${inspHistory.length}`:''})</div>{(()=>{const list=inspHistory.filter(r=>!inspFilter||r.info?.property===inspFilter);return list.length===0?<div className="empty"><div style={{fontSize:36,marginBottom:10}}>📋</div><div style={{fontWeight:600}}>{inspFilter?'No matches':'No inspections yet'}</div></div>:list.map(rec=>(<div className="hcard" key={rec.id}><div className="hcard-top"><div><div className="hcard-prop">{rec.info.property||'Unnamed'}</div><div className="hcard-date">{new Date(rec.savedAt).toLocaleDateString()}</div></div>{rec.summary.status&&<span className={`badge ${getColor(rec.summary.status==='Good'?'Pass':rec.summary.status==='Monitor'?'M':'Fail')}`}>{rec.summary.status}</span>}</div><div className="hcard-meta"><span>🤖 {rec.info.model?.join(' + ')||'—'}</span><span>S/N: {rec.info.serial||'—'}</span></div><div className="hcard-actions"><div className="hbtn" onClick={()=>{setViewingInsp(rec);setStep('receipt');}}>View</div><div className="hbtn" onClick={()=>handleInspPDF(rec)}>⬇ PDF</div><div className="hbtn" onClick={e=>deleteInsp(rec.id,e)}>Delete</div></div></div>));})()}</div>}
      ...
      {tab==='service'&&<>
        {step==='home'&&<div className="content"><button className="btn" style={{marginTop:0}} onClick={()=>{setSR(initSR());setStep('sr_form');}}>+ New Service Report</button>{allPropertyNames().length>0&&(<><div className="sec">Filter</div><div className="filter-row"><select value={srFilter} onChange={e=>setSRFilter(e.target.value)}><option value="">All Properties</option>{allPropertyNames().map(n=><option key={n} value={n}>{n}</option>)}</select>{srFilter&&<button className="filter-clear" onClick={()=>setSRFilter('')}>Clear</button>}</div></>)}<div className="sec">Saved ({srHistory.filter(r=>!srFilter||r.property===srFilter).length}{srFilter?` of ${srHistory.length}`:''})</div>{(()=>{const list=srHistory.filter(r=>!srFilter||r.property===srFilter);return list.length===0?<div className="empty"><div style={{fontSize:36,marginBottom:10}}>🔧</div><div style={{fontWeight:600}}>{srFilter?'No matches':'No reports yet'}</div></div>:list.map(rec=>(<div className="hcard sr-card" key={rec.id}><div className="hcard-top"><div><div className="hcard-prop">{rec.property||'Unnamed'}</div><div className="hcard-date">{new Date(rec.savedAt).toLocaleDateString()}</div></div><span className="badge ba">Service</span></div><div className="hcard-meta"><span>🤖 {rec.machineName||'—'}</span>{rec.linkedComponent&&<span style={{color:C.attention}}>⚠ {rec.linkedComponent}</span>}</div><div className="hcard-actions"><div className="hbtn" onClick={()=>{setViewingSR(rec);setStep('sr_receipt');}}>View</div><div className="hbtn" onClick={()=>handleSRPDF(rec)}>⬇ PDF</div><div className="hbtn" onClick={()=>emailSR(rec)}>📧</div><div className="hbtn" onClick={e=>deleteSR(rec.id,e)}>Delete</div></div></div>));})()}</div>}
